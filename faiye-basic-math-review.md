...
Proof. Suppose that there are finitely many prime numbers. If the prime numbers are represented in terms of $p_1, p_2, ... p_n$ where $n$ is a finite integer, then let $m$ be a product of all prime numbers plus $1$.  Notice how if we divide $m$ by a prime number that is not equal to $m$ or $1$, we will always get a remainder of $1$. Then by definition $m$ is a prime number, but if $m$ is a prime number, then surely $m$ must contain itself in the product of all primes $p_1p_{2}... p_n$. Since $m$ cannot contain itself, we have reached a contradiction, thus there are infinitely many prime numbers. 
